...
namespace TripleA.Editor.AnimationEvents
{
    /// <summary>
    ...
    public class AnimationEventStateBehaviourEditor : UnityEditor.Editor {
        ...
        void PreviewAnimationClip(AnimationEventStateBehaviour stateBehaviour) {
            ...
            if (motion is AnimationClip clip) {
                m_previewTime = stateBehaviour.triggerTime * clip.length;
                AnimationMode.SampleAnimationClip(Selection.activeGameObject, clip, m_previewTime);
            }
        }
        ...
        void SampleBlendTreeAnimation(AnimationEventStateBehaviour stateBehaviour, float normalizedTime) {
            ...
            m_playableGraph = PlayableGraph.Create("BlendTreePreviewGraph");
            m_mixer = AnimationMixerPlayable.Create(m_playableGraph, 1);

            var output = AnimationPlayableOutput.Create(m_playableGraph, "Animation", animator);
            ...
            for (int i = 0; i < blendTree.children.Length; i++) {
                ...
                clipPlayables[i] = AnimationClipPlayable.Create(m_playableGraph, clip);
            }
            ...
            m_mixer.SetInputCount(clipPlayables.Length);
            for (int i = 0; i < clipPlayables.Length; i++) {
                m_mixer.ConnectInput(i, clipPlayables[i], 0);
                m_mixer.SetInputWeight(i, weights[i]);
            }

            AnimationMode.SamplePlayableGraph(m_playableGraph, 0, normalizedTime);
        }
        ...
        bool Validate(AnimationEventStateBehaviour stateBehaviour, out string errorMessage) {
            ...
            m_previewClip = GetAnimationClipFromMotion(matchingState.state?.motion);
